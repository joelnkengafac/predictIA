...
# CORRECTIF PERMISSIONS : ces vues étaient protégées par @decideur_required
# (Administrateur + Décideur uniquement), ce qui empêchait l'Analyste
# budgétaire de consulter ses propres résultats de prévision et d'utiliser
# l'assistant IA — alors que la matrice de permissions cible l'y autorise
# explicitement ("consulter les résultats", "chatbot Grok" = accessible aux
# 3 rôles). Remplacé par @tous_roles_required (Admin + Analyste + Décideur).
# Le Décideur conserve sa capacité existante à générer une prévision à partir
# des modèles déjà entraînés (fonctionnalité de consultation/interprétation,
# distincte de l'ENTRAÎNEMENT des modèles qui reste, lui, réservé à
# Administrateur + Analyste dans forecasting/views.py).
from accounts.decorators import tous_roles_required
...
from .services.grok_service import AssistantIndisponible, poser_question
