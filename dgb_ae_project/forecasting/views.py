...
from accounts.decorators import analyste_required, tous_roles_required
...
def lancer_entrainement(request):
    ...
    if resultat.millesime_exploite:
        messages.info(
            request,
            "exMillesime a été détecté comme fiable sur ce dataset (correspondance constante avec "
            "l'année civile) et a été exploité comme variable explicative supplémentaire pour "
            "XGBoost et LightGBM.",
        )
...
def page_comparaison(request):
    # CORRECTIF PERMISSIONS : cette vue était réservée à @analyste_required
    # (Admin + Analyste), alors que le menu du Décideur (core/context_processors.py,
    # entrée "Indicateurs") pointe précisément vers cette page — un Décideur qui
    # cliquait dessus recevait donc un 403, alors que la matrice cible prévoit
    # "Comparer modèles : ✅/lecture" pour ce rôle. Page strictement en lecture
    # seule (aucune action d'entraînement ici), donc sans risque à ouvrir aux 3 rôles.
