...
`exMillesime` (exercice budgétaire) est exploité de façon CONDITIONNELLE
comme variable explicative des modèles tabulaires (XGBoost/LightGBM) : voir
`detecter_offset_millesime()` et `get_feature_columns()` plus bas — il n'est
ajouté que lorsque sa correspondance avec l'année civile est constante et
sans valeur manquante sur le dataset importé, ce qui garantit qu'il reste
extrapolable de façon fiable pour les prévisions futures.
...
def lire_fichier_dataset(fichier, extension: str) -> pd.DataFrame:
    """Lit un fichier importé (CSV ou Excel .xlsx) en DataFrame brut, selon
    son extension — point d'entrée unique utilisé par datasets/views.py, pour
    que l'ajout d'un futur format supplémentaire ne se fasse qu'ici.
    """
    if extension == ".xlsx":
        try:
            fichier.seek(0)
    ...
    return lire_csv_intelligent(fichier)
...
def get_feature_columns(inclure_millesime: bool = False) -> list[str]:
    ...
    `ex_millesime` (exercice budgétaire) — EXPLOITÉ DE FAÇON CONDITIONNELLE :
    sa correspondance avec l'année civile n'est PAS garantie constante sur tous
    les jeux de données (un exercice budgétaire peut chevaucher deux années
    civiles selon les conventions comptables retenues). L'utiliser à l'aveugle
    comme variable numérique romprait le pipeline dès qu'un dataset ne suit pas
    une convention stable, et une valeur non extrapolable de façon fiable pour
    le futur n'a pas sa place parmi les features.
    On ne l'ajoute donc ici QUE si l'appelant (voir pipeline.py ->
    `detecter_offset_millesime`) a préalablement vérifié, sur le dataset
    concerné, que exMillesime = année_civile + décalage_constant sur 100% des
    lignes — auquel cas sa valeur future est déductible avec certitude à
    l'horizon de prévision (même formule), et elle apporte une information
    métier réelle : le fait de savoir dans quel exercice budgétaire une
    observation se situe peut capturer des effets de cycle budgétaire
    (ex. sous-engagement fréquent en début d'exercice, rattrapage en fin
    d'exercice) que 'annee'/'mois' seuls ne représentent pas explicitement.
    """
    colonnes = ["chapitre", "annee", "mois", "trimestre", "trend_index"] + [
        f"lag_{lag}" for lag in LAG_PERIODS
    ]
    if inclure_millesime:
        colonnes.append("ex_millesime")
    return colonnes


def detecter_offset_millesime(df: pd.DataFrame) -> int | None:
    """Détermine si `ex_millesime` peut être exploité comme variable
    explicative fiable pour CE dataset précis.

    Retourne le décalage entier constant `d` tel que
    `ex_millesime == annee + d` sur 100% des lignes du dataset nettoyé, ou
    `None` si la colonne est absente, incomplète, ou si le décalage varie
    (dans ce cas exMillesime reste disponible pour l'affichage/l'export mais
    n'est PAS utilisé comme feature — voir get_feature_columns()).

    Appelée sur le DataFrame déjà produit par `nettoyer_et_preparer()`
    (colonnes `annee` et `ex_millesime` garanties présentes).
    """
    if "ex_millesime" not in df.columns or "annee" not in df.columns:
        return None
    if df["ex_millesime"].isna().any():
        # Décalage non extrapolable de façon fiable si des lignes n'ont pas
        # de millésime renseigné (dataset au format simplifié, ou format
        # complet avec des valeurs manquantes) : on refuse plutôt que de
        # deviner/imputer une valeur budgétaire.
        return None

    ecarts = (df["ex_millesime"].astype(int) - df["annee"].astype(int)).unique()
    if len(ecarts) != 1:
        return None
    return int(ecarts[0])
