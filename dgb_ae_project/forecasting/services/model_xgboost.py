...
def entrainer_xgboost(
    ...
    — n'ajoute exMillesime comme variable explicative que si le pipeline appelant
    a confirmé sa fiabilité pour ce dataset précis.
    """
...
def predire_futur_xgboost(
    model: XGBRegressor, X_future: pd.DataFrame, inclure_millesime: bool = False
) -> np.ndarray:
    ...
    `entrainer_xgboost` pour ce modèle (mêmes colonnes de features des deux côtés).
    """
    features = get_feature_columns(inclure_millesime=inclure_millesime)
