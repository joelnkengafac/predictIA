...
from .preprocessing import detecter_offset_millesime, get_feature_columns, nettoyer_et_preparer
...
class ResultatEntrainement:
    ...
    # exMillesime (exercice budgétaire) — voir preprocessing.detecter_offset_millesime().
    # `millesime_exploite` indique si exMillesime a pu être ajouté comme variable
    # explicative pour les modèles tabulaires (XGBoost/LightGBM) POUR CE DATASET ;
    # `millesime_offset` est le décalage constant exMillesime = année + offset,
    # nécessaire pour reconstruire cette valeur aux dates futures lors de la
    # génération des prévisions (generer_previsions_futures).
    millesime_exploite: bool = False
    millesime_offset: int | None = None
...
def entrainer_tous_les_modeles(df_brut: pd.DataFrame, train_ratio: float = 0.8) -> ResultatEntrainement:
    ...
    # exMillesime : n'est utilisé comme variable explicative que si ce dataset
    # précis présente une correspondance constante et sans valeur manquante
    # avec l'année civile (voir detecter_offset_millesime) — sinon on le laisse
    # disponible uniquement pour le stockage/l'affichage, jamais pour l'entraînement.
    offset_millesime = detecter_offset_millesime(df)
    inclure_millesime = offset_millesime is not None
    if inclure_millesime:
        logger.info(
            "exMillesime exploité comme variable explicative (décalage constant détecté : "
            "exMillesime = année + %d).", offset_millesime,
        )
    else:
        logger.info(
            "exMillesime non exploité comme variable explicative pour ce dataset "
            "(colonne absente, incomplète, ou décalage non constant avec l'année civile)."
        )
    ...
    resultat = ResultatEntrainement(
        ...
        millesime_exploite=inclure_millesime,
        millesime_offset=offset_millesime,
    )

    # --- XGBoost ---
    try:
        model_xgb, y_pred_xgb, duree_xgb = entrainer_xgboost(
            split.train, split.test, inclure_millesime=inclure_millesime
        )
    ...
    # --- LightGBM ---
    try:
        model_lgbm, y_pred_lgbm, duree_lgbm = entrainer_lightgbm(
            split.train, split.test, inclure_millesime=inclure_millesime
        )
...
def generer_previsions_futures(
    ...
    if nom_modele == "LSTM":
        ...
        for pas in range(1, horizon + 1):
            ...
            if resultat.millesime_exploite:
                # Reconstruction déterministe de exMillesime à la date future,
                # à partir du décalage constant détecté lors de l'entraînement
                # (exMillesime = année civile + offset) — voir
                # preprocessing.detecter_offset_millesime().
                ligne_features["ex_millesime"] = date_future.year + resultat.millesime_offset
            ...
            features_attendues = get_feature_columns(inclure_millesime=resultat.millesime_exploite)
            if X_future[features_attendues].isna().any(axis=None):
            ...
            if nom_modele == "XGBoost":
                pred = predire_futur_xgboost(
                    resultat._artefacts["XGBoost"], X_future, inclure_millesime=resultat.millesime_exploite
                )[0]
            else:
                pred = predire_futur_lightgbm(
                    resultat._artefacts["LightGBM"], X_future, inclure_millesime=resultat.millesime_exploite
                )[0]
