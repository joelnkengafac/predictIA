...
Le Décideur dispose d'un chat contextualisé sous chaque prévision générée
(`predictions/services/assistant_ia.py`), basé sur l'API Anthropic. Pour
l'activer :
```
...
| Analyste       | Import de données, entraînement, comparaison, prévisions |
| Décideur       | Consultation des prévisions et rapports (interface simplifiée) |
